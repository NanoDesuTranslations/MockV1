// MockStories.js
// Contains functions (prototype) for generating nonsense HTML text suitable for filling in a story
// with logic for including sample pictures

// A short array of known image URLs (from sasami-san)
arImages = [
     "https://sasamisanthetranslation.files.wordpress.com/2013/03/1-1.png",
     "https://sasamisanthetranslation.files.wordpress.com/2014/05/sg_10023.png",
     "https://sasamisanthetranslation.files.wordpress.com/2014/05/sg_10052.png"
];

function rndImageURL() {
    var n = nPage % arImages.length;
    return arImages[n];
}

// Define an array of nonsense sentences for generating "greek" text.
arLorem = [ "Lorem ipsum dolor sit amet, consectetuer adipiscing elit.",
...
function loremStory(idx) {
    var s = "";  // String.  Will contain HTML for the dummy content.
    // assert: idx < "lorem ipsum" array length.
    // s = "<h3>Chapter " + idx + ": " + arLorem[idx++] + "</h3>";
    var nParImage = (idx + 5) % 8; // So the image is placed in a fixed location for each value of idx, and could be after 0-7 paragraphs.
    var nPar = 0;
    var nLines = 0;

    // Loop through, creating paragraphs.  Create the image when the right number of preceding paragraphs have been written.
    while (idx < arLorem.length && nLines < 80) {
        if (nPar == nParImage) {
            var sUrlImage = rndImageURL();
            sImgTag = "<img class=\"img-responsive center-block\" src=\""
               + sUrlImage
               + "\" width=\"600\" \\>";
            s += sImgTag + "\n";
        }
    ...
    //s += "<p>";
    //var i; var nPar = 0;
    //for (i = 1; i <= 80 && idx < arLorem.length; i++) {
    //    s += arLorem[idx++];
    //    if (Math.random() < 0.2 && idx < arLorem.length) {
    //        s += "<\p> \n <p>";
    //    }
    //}
    //s += "</p>";
...
// inline script figures out the page identifier
var nPage = (function () {
    var sQuery = location.search; // query string
    if (sQuery.indexOf("chap=") >= 0) {
        return Number.parseInt(sQuery.substring(sQuery.indexOf("chap=") + "chap=".length));
    } else return 0;
})();
